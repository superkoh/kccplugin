 ...
 * Contract
 * --------
 * buildVariant(sourceText, { anchor, snippet, sentinel, label })
 *
 ...
 *   snippet     exact-clause rewrite(s): { find, with } or an array of
 *               them; `find` must occur exactly once.
 *   sentinel    arm-unique token substituted into the doc's
 *               `<!-- …sentinel: … -->` line, so the arm can be
...
// Each ablatable document names its own marker (`kcc-core-sentinel`,
// `kcc-dev-core-<skill>-sentinel`, …) — so the prefix is matched loosely
// and only the token is rewritten.
...
export function buildVariant(
  ...
  if (!SENTINEL_RE.test(sourceText)) {
    throw new Error(`no sentinel line in the source document (arm "${label}")`);
  }
