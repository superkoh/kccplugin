/**
 * The tools no probe may use, shared by every probe definition.
 *
 ...
 *
 * This lived as a copy-pasted literal in each probe module until a real gap
 * cost a run: a probe voided with "escaped the lockdown via ListAgents"
 * because the agent-roster tools were on nobody's list. Six copies were
 * identical and one had already drifted, so the fix had to land seven times
 * to be real. It is one module now.
 *
 * A run that calls something outside its probe's `expectedTools` is voided
 * rather than scored (see screenRun in score.mjs), so a new escape hatch
 * shows up as an invalid record naming the tool — add it here when that
 * happens.
 */
