#!/usr/bin/env bash
# kcc-dev-core SubagentStart hook: CONDITIONALLY inject the subagent
# variant of the development-discipline principles into every spawned
# subagent's context, but only when the session is operating inside a
# software project.
#
...
#     an empty additionalContext, so a broken hook never prevents a
...
#     SubagentStart hook's stdin JSON, then fall back to $PWD. Either path
...
text_file="$plugin_root/context/dev-principles-subagent.md"
...
# ---------------------------------------------------------------------
kcc_emit_file "SubagentStart" "$text_file"
